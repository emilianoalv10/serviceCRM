...
router.get('/', (req, res) => {
  const { client_id, category, paid, from, to } = req.query;
  const where = [];
  const params = [];
...
router.post('/', (req, res) => {
  const { client_id, category, description, service_date, price, paid } = req.body;
  ...
  const info = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(client_id, category, description || null, service_date, priceNum, paidNum, paidAt);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`
    UPDATE services SET client_id = ?, category = ?, description = ?, service_date = ?, price = ?, paid = ?, paid_at = ?
    WHERE id = ?
  `).run(
